...
import { createJWS, verifyJWS } from './jws';
...
export function Ceramic() {
  ...
  const updateStream = async () => {
    ...
    await document.replace({ ...(document.content as any), contentId: '' });
  };
  ...
  const fromStreamId = () => {
    console.log(
      StreamID.fromString('kjzl6hvfrbw6c6q29co83n8av8krlst35wbojuhxxs2nfuykirbq4nndjjvdu3q'),
    );
  };

  const createCeramicJWS = async () => {
    jws = await createJWS({ code: 'code', requestPath: '/api/v0/admin/models', ceramic });
    console.log(jws);
  };

  const verifyCeramicJWS = async () => {
    const res = await verifyJWS(ceramic, jws);
    console.log(res);
  };
  ...
  return (
    <div>
      ...
      <button onClick={createCeramicJWS}>signWithJWS</button>
      <button onClick={verifyCeramicJWS}>verify</button>
